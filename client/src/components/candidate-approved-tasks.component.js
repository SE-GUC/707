...
export default class approvedtasks extends Component {
  constructor(props) {
    ...
    this.state = {
      tasks: [],
      updatedCycle: { description: "", status: "", percentage: "" },
      update: false,
      task: {}
    };
  }
  componentDidMount() {
    ...
    axios
      .get("http://localhost:5000/api/candidates/approvedTasks", {
        headers: {
          Authorization: token
        }
      })
  ...
  onSubmit(taskID) {
    ...
    axios
      .get("http://localhost:5000/api/candidates/project/task/" + taskID, {
        headers: {
          Authorization: token
        }
      })
      .then(res => {
        ...
        axios
          .put(
            "http://localhost:5000/api/candidates/project/tasks/" +
              mainProject._id +
              "/" +
              taskID,
            this.state.updatedCycle,
            {
              headers: {
                Authorization: token
              }
          ...
          .then(res => {
            ...
            axios
              .put(
                "http://localhost:5000/api/candidates/update/tasks",
                {},
                {
                  headers: {
                    Authorization: token
          ...
          {this.state.task.taskcycle.map(cycle => (
            <p>
              ...
              <input
                ...
                onClick={this.onSubmit.bind(this, this.state.task._id)}
              />
            </p>
          ))}
        </ul>
