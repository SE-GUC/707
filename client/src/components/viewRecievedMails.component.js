...
onSubmit(e) {
  ...
  axios.post("http://localhost:5000/api/emails",email,{  headers: {
      ...
      componentDidMount() {
        const cookies= new Cookies();
         const token= cookies.get('token')
        axios.get("http://localhost:5000/api/emails/received", {  headers: {
          Authorization: token}
        })
          .then(res => {
    ...
    onSubmit3(id) {
        ...
        axios.get('http://localhost:5000/api/emails/received/'+ id, { headers: {
        ...
        return (
          <ul>
                ...
                { this.state.emails.map(sent => 
                  <tbody>
                 <tr >
                <td>{sent.senderEmail}<br></br></td>   
                <td>{sent.subject}<br></br></td>   
                <td>{sent.content}<br></br></td>  
                <td>{sent.emailType}<br></br></td > 
                
                <td><button type="button" className="btn btn-danger" onClick={this.onSubmit2.bind(this, sent._id)}>Delete</button><br></br></td >
                    
                <td><button type="button" className="btn btn-danger" onClick={this.onSubmit3.bind(this, sent._id)}>view mail</button><br></br></td > 
