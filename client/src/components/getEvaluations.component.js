...
export default class getEvaluations extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        "http://localhost:5000/api/consultancies/certificate/evaluationTests/:certificateID",
        {
          headers: {
            Authorization: token
          }
  ...
  onSubmit(id) {
    ...
    axios
      .get(
        "http://localhost:5000/api/consultancies/certificate/evaluationTest/" +
          id,
        {
          headers: {
            Authorization: token
    ...
    return (
      <ul>
        {this.state.evaluations.map(person => (
          <li>
            <p>
              evaluation type: {person.type}
              <br />
              evaluation content: {person.content}
              <br />
              <button
                type="button"
