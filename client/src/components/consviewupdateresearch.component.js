...
export default class viewupdateconstresearch extends Component {
  ...
  componentDidMount() {
    ...
    if (usertype !== "consultancy") {
      ...
      axios
        .get(
          "http://localhost:5000/api/consultancies/researches/" +
            { research }.research,
          {
            headers: {
              Authorization: token
  ...
  updateResearch(e) {
    ...
    if (this.state.temp) {
      ...
      axios.put(
        "http://localhost:5000/api/consultancies/research/" +
          { research }.research,
        researchnew,
        {
          headers: {
  ...
  render() {
    return (
      <ul>
        ...
        <br />
        <button type="update" onClick={event => this.updateResearch(event)}>
          Update
        </button>
        <br />
      </ul>
