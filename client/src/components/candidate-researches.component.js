...
export default class candidateresearches extends Component {
    ...
    this.state = {
      ...
      componentDidMount() { 
        ...
        console.log(token)
        axios.get('http://localhost:5000/api/candidates/researches', { headers: {
      ...
      onSubmit(id) {
        ...
        const token= cookies.get('token');
        console.log(token)
        axios.get('http://localhost:5000/api/candidates/research/'+ id, { headers: {
            Authorization: token.data}
          })
          .then(res => {
        ...
        return (
          ...
          <ul>
               { this.state.researches.map(research => <li>
                ...
                <button type="button" className="btn btn-primary" onClick={this.onSubmit.bind(this, research._id)}>View</button>
                </p> 
                </li>
                  )}
