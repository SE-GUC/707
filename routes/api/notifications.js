...
router.post("/", async (req, res) => {
    try {
      ...
      if(!adminId)  return res.status(404).send({error: "This sender does not exist"});

      const candidateId= await Candidate.findById(req.body.receiverID)
      if(!candidateId)  return res.status(404).send({error: "This receiver does not exist"});
  ...
  if(!candidate) return res.status(404).send({error: "This partner does not exist"});
  ...
  router.get("/:id", async (req, res) => {
    try {
      ...
      if (!notification)
        return res.status(404).send({error: "This notification does not exist"});
