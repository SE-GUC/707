...
router.post("/:id", async (req, res) => {
  try {
    ...
    if(!adminId) {
      ...
      if(!partnerId){  return res.status(404).send({error: "This sender does not exist"});

    }else{
      const adminId= await Admin.findById(req.body.receiverID)
      if(!adminId)  return res.status(404).send({error: "This receiver does not exist"});

    }
  }
    else{
      const partnerId= await Partner.findById(req.body.receiverID)
      if(!partnerId)  return res.status(404).send({error: "This receiver does not exist"});
    }
...
//View my messages(sent and received)
  ...
  if(!admin) {
    ...
    if(!partner) return res.status(404).send({error: "This user does not exist"});
    

  }
 
  const myMessages= await Message.find({$or: [
    {senderID: id},
    {receiverID: id}
]})
