...
router.put("/accept/:id",async (req, res) => {
    try {
     ...
     return res.status(404).send({ error: "This project is already assigned" });

       project.assignedCandidate= candidateID
       Project.findByIdAndUpdate(project.id,project, { 'new': true }, function(err, updatedProject){
        if(!err)
            res.json({msg: 'Your project has been assigned successfully',data: updatedProject})
        else
