...
router.get("/:id", async (req, res) => {
    try{
        ...
        if(!certificate) return res.status(404).send({error: 'This certificate does not exist'})
...
router.put('/:id', async (req,res) => {
    try {
     ...
     if (isValidated.error) return res.status(400).send({ error: isValidated.error.details[0].message })
      Certificate.findByIdAndUpdate(id,req.body, { 'new': true }, function(err, updatedCertificate){
        if(!err)
            res.json({msg: 'Your certificate has been updated successfully',data: updatedCertificate})
        else
            res.json({msg: err.message})
    })
...
router.delete("/:id", async (req, res) => {
    try {
      ...
      res.json({ msg: "Your certificate has been deleted successfully",  data: deletedCertificate });
    } catch (error) {
      res.json({ msg: error.message });
    }
  });
