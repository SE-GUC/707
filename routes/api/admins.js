...
router.get("/viewProjects/:id", async (req, res) => {
  ...
  const admin = await Admin.findById(req.params.id);
  if(!admin){
    return res.status(404).send({ error: "This admin does not exsist" });
  }
  res.json({data:admin.projects});
}catch (error) {
  res.json({ msg: error.message });
} 
});
...
router.put("/acceptProjects/:id/:id2/:id3", async (req, res) => {
  ...
  if(!admin){
    ...
    Project.findByIdAndUpdate(req.params.id2, {approveAdmin:true}, function() {});
...
router.put("/rejectProjects/:id/:id2/:id3", async (req, res) => {
  ...
  if(!admin){
    ...
    Project.findByIdAndUpdate(req.params.id2, {approveAdmin:false}, function() {});
    Partner.findByIdAndUpdate(
      req.params.id3,
      {
        $push:{
...
router.post("/createProjectAttributes/:id/:idpartner", async (req, res) => {
  try {
    ...
    // );
    Partner.updateOne(
      {_id:partner._id,"projects._id":project._id},{$set:{"projects.$.tasks" : task}
