...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!lifecycle)
      return res.status(404).send({ error: "This lifecycle does not exist" });
  ...
  try {
    ...
    if (isValidated.error)
      return res
        .status(400)
        .send({ error: isValidated.error.details[0].message });
    Lifecycle.findByIdAndUpdate(id, req.body, { new: true }, function(
      err,
      updatedLifecycle
    ) {
      if (!err)
        res.json({
          msg: "Your lifecyle has been updated successfully",
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.json({
      msg: "Your lifecycle has been deleted successfully",
      data: deletedLifecycle
    });
  } catch (error) {
    res.json({ msg: error.message });
  }
