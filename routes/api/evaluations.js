...
router.post("/", async (req, res) => {
  try {
    ...
    res.json({
      msg: "Evaluation was created successfully",
      data: newEvaluation
    });
  } catch (error) {
    console.log(error);
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!evaluation)
      return res.status(404).send({ error: "This evaluation does not exist" });
  ...
  try {
    ...
    if (isValidated.error)
      return res
        .status(400)
        .send({ error: isValidated.error.details[0].message });
    Evaluation.findByIdAndUpdate(id, req.body, { new: true }, function(
      err,
      updatedEvaluation
    ) {
      if (!err)
        res.json({
          msg: "Your evaluation has been updated successfully",
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.json({
      msg: "Your evaluation has been deleted successfully",
