...
router.post("/:id", async (req, res) => {
  try {
    ...
    const receiverConversation = { SentMessages: [], RecievedMessages: [], receiverEmail: senderConsultancy.email };
    if (receiverAdmin != null) {
      Consultancy.update({ _id: senderID }, { $push: { conversations: senderConversation } }, function () {});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.json({ data: senderConsultancy.conversations });
  } catch (err) {
...
router.get("/:id/:email", async (req, res) => {
  try {
    ...
    for(i = 0 ; i < senderConsultancy.conversations.length ; i++)
      if(senderConsultancy.conversations[i].receiverEmail === email)
        return res.json({ data: senderConsultancy.conversations[i] });
  } catch (err) {
    res.json({ msg: err.message });
  }
});
  ...
  try {
    ...
    for(i = 0 ; i < senderConsultancy.conversations.length ; i++)
      if(senderConsultancy.conversations[i].receiverEmail === receiverEmail)
        ...
        senderConversations[i] = senderConsultancy.conversations[i];
    if(!flag)
    ...
    if (receiverAdmin != null) {
      for(i = 0 ; i < receiverAdmin.conversations.length ; i++)
        if(receiverAdmin.conversations[i].receiverEmail !== senderConsultancy.email)
          receiverConversations[i] = receiverAdmin.conversations[i];
      Consultancy.update({ _id: senderID }, { conversations: senderConversations }, function () {});
    ...
    if (receiverCandidate != null) {
      for(i = 0 ; i < receiverCandidate.conversations.length ; i++)
        if(receiverCandidate.conversations[i].receiverEmail !== senderConsultancy.email)
    ...
    if (receiverPartner != null) {
      for(i = 0 ; i < receiverPartner.conversations.length ; i++)
        if(receiverPartner.conversations[i].receiverEmail !== senderConsultancy.email)
    ...
    if (receiverConsultancy != null) {
      for(i = 0 ; i < receiverConsultancy.conversations.length ; i++)
        if(receiverConsultancy.conversations[i].receiverEmail !== senderConsultancy.email)
