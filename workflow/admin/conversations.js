...
router.post("/:id", async (req, res) => {
  try {
    ...
    const receiverConversation = { SentMessages: [], RecievedMessages: [], receiverEmail: senderAdmin.email };
    if (receiverAdmin != null) {
      Admin.update({ _id: senderID }, { $push: { conversations: senderConversation } }, function () {});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.json({ data: senderAdmin.conversations });
  } catch (err) {
  ...
  try {
    ...
    for(i = 0 ; i < senderAdmin.conversations.length ; i++)
      if(senderAdmin.conversations[i].receiverEmail === email)
        return res.json({ data: senderAdmin.conversations[i] });
  } catch (err) {
    res.json({ msg: err.message });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (receiverAdmin != null) {
      ...
      Admin.update({ _id: senderID }, { conversations: senderConversations }, function () {});
