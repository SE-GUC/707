...
router.post("/:id", async (req, res) => {
  try {
    ...
    const receiverConversation = { SentMessages: [], RecievedMessages: [], receiverEmail: senderCandidate.email };
    if (receiverAdmin != null) {
      Candidate.update({ _id: senderID }, { $push: { conversations: senderConversation } }, function () {});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.json({ data: senderCandidate.conversations });
  } catch (err) {
...
router.get("/:id/:email", async (req, res) => {
  try {
    ...
    for(i = 0 ; i < senderCandidate.conversations.length ; i++)
      if(senderCandidate.conversations[i].receiverEmail === email)
        return res.json({ data: senderCandidate.conversations[i] });
  } catch (err) {
    res.json({ msg: err.message });
  }
});
  ...
  try {
    ...
    for(i = 0 ; i < senderCandidate.conversations.length ; i++)
      if(senderCandidate.conversations[i].receiverEmail === receiverEmail)
        ...
        senderConversations[i] = senderCandidate.conversations[i];
    if(!flag)
    ...
    if (receiverAdmin != null) {
      for(i = 0 ; i < receiverAdmin.conversations.length ; i++)
        if(receiverAdmin.conversations[i].receiverEmail !== senderCandidate.email)
          receiverConversations[i] = receiverAdmin.conversations[i];
      Candidate.update({ _id: senderID }, { conversations: senderConversations }, function () {});
    ...
    if (receiverCandidate != null) {
      for(i = 0 ; i < receiverCandidate.conversations.length ; i++)
        if(receiverCandidate.conversations[i].receiverEmail !== senderCandidate.email)
    ...
    if (receiverPartner != null) {
      for(i = 0 ; i < receiverPartner.conversations.length ; i++)
        if(receiverPartner.conversations[i].receiverEmail !== senderCandidate.email)
    ...
    if (receiverConsultancy != null) {
      for(i = 0 ; i < receiverConsultancy.conversations.length ; i++)
        if(receiverConsultancy.conversations[i].receiverEmail !== senderCandidate.email)
